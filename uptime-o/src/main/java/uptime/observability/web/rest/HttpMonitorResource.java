...
public class HttpMonitorResource {
    ...
    /**
     * {@code GET  /http-monitors/count} : get the count of http monitors.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the count.
     */
    @GetMapping("/count")
    public ResponseEntity<java.util.Map<String, Long>> getHttpMonitorCount() {
        LOG.debug("REST request to get HttpMonitor count");
        long count = apiMonitorRepository.count();
